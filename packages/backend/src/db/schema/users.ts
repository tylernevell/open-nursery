...
export const caregiversTable = pgTable(
  "caregivers",
  {
    ...
    relationship: t.varchar("relationship", {
...
export const babiesTable = pgTable(
  "babies",
  {
    id: t.integer().primaryKey().generatedAlwaysAsIdentity(),
...
export const babyToCaregiversTable = pgTable(
  "baby_to_caregivers",
  {
    babyId: t.integer("baby_id")
      .references(() => babiesTable.id)
      .notNull(),
    caregiverId: t.integer("caregiver_id")
  ...
  (table) => [
    t.primaryKey({ columns: [table.babyId, table.caregiverId] })
  ]
).enableRLS();
