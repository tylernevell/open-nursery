...
export const eventsTable = pgTable(
  'events',
  {
    ...
    babyId: integer().references(() => babiesTable.id).notNull(),
    caregiverId: integer().references(() => caregiversTable.id).notNull(),
...
export const sleepEventsTable = pgTable(
  'sleep_events',
  {
    eventId: integer().primaryKey().references(() => eventsTable.id, { onDelete: 'cascade' }),
...
export const feedingEventsTable = pgTable(
  'feeding_events',
  {
    eventId: integer().primaryKey().references(() => eventsTable.id, { onDelete: 'cascade' }),
...
export const diaperEventsTable = pgTable(
  'diaper_events',
  {
    eventId: integer().primaryKey().references(() => eventsTable.id, { onDelete: 'cascade' }),
